...
- 适配器模板：`runtime/cellbot_adapter.py`
- 适配器工厂：`runtime/robot_factory.py`（支持注册新 family）
- 适配文档（英文）：`runtime/docs/robot_adaptation.md`
- 适配文档（中文）：`runtime/docs/robot_adaptation_zh.md`
...
```bash
pip install -r runtime/requirements.rekep-extra.txt
# 可选：solver 模式所需
pip install -r runtime/requirements.rekep-optional.txt
```
...
- `REKEP_TOOL_ROOT`：ReKep 根目录（默认即当前 `runtime/`）
...
- `REKEP_XTRAINER_SDK_DIR`：本地 SDK 路径（默认 `runtime/third_party/dobot_xtrainer`）
...
```bash
# 机器人 ZMQ 服务（默认 6001）
python runtime/third_party/dobot_xtrainer_remote/experiments/launch_nodes.py \
  --hostname 0.0.0.0 --robot-port 6001

# RealSense ZMQ 流服务（默认 7001 / topic=realsense）
python runtime/third_party/dobot_xtrainer_remote/experiments/launch_realsense_server.py \
