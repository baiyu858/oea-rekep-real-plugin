...
适配器需满足 `runtime/robot_adapter.py`：
...
`runtime/robot_factory.py` 已支持可扩展注册：
...
`runtime/dobot_bridge.py` 现在支持通用参数（且兼容旧 `dobot_*` 参数）：
...
建议复用或镜像 `runtime/real_calibration/` 的目录结构来维护你自己的标定配置。
